...
class PlanExecutor:
    ...
    def __init__(self, session_id: str, initial_instruction: str, original_user_input: str, dependencies: dict):
        ...
        # --- Properties from WorkflowExecutor ---
    ...
    async def run(self):
        for i in range(self.max_steps):
            if self.state in [self.AgentState.SUMMARIZING, self.AgentState.DONE, self.AgentState.ERROR]:
                break
            try:
                # --- ROUTER: Decide which execution path to take ---
                if self.is_workflow:
                    # If it's a workflow, run the workflow state machine
                    async for event in self._run_workflow():
                        yield event
                    # If the workflow transitions to summarizing, break the main loop
                    if self.state == self.AgentState.SUMMARIZING:
                        break
                else:
                    # Otherwise, run the original non-workflow logic
                    if self.state == self.AgentState.DECIDING:
    ...
    # -------------------------------------------------------------------------
    # --- WORKFLOW EXECUTION LOGIC (Moved from WorkflowExecutor) ---
    # -------------------------------------------------------------------------
    ...
    async def _get_next_workflow_action(self, current_phase_goal: str, relevant_tools: list[str]) -> tuple[dict | str, int, int]:
        """
        ...
        try:
            ...
            while self.current_phase_index < len(self.meta_plan):
                ...
                while True:
                    ...
                    yield self._format_sse({"step": "Calling LLM", "details": reason_action})
                    
                    next_action, input_tokens, output_tokens = await self._get_next_workflow_action(phase_goal, relevant_tools)
                    ...
                    yield self._format_sse({"step": "Tool Execution Intent", "details": next_action}, "tool_result")
            ...
            yield self._format_sse({"error": "Workflow Execution Failed", "details": str(e)}, "error")
            self.state = self.AgentState.ERROR
            return
