...
PROVIDER_SYSTEM_PROMPTS = {
    "Google": (
        ...
        "1.  **CHOOSE ONE CAPABILITY:** First, review both lists and select the single best capability (either a prompt or a tool) that can can fulfill the user's request. If a prompt can solve the entire request, you MUST choose the prompt.\n\n"
        ...
        "If the user's request cannot be fulfilled by any of the available tools or prompts, or if it is a general conversational query that does not pertain to database operations (and is not a simple greeting handled by the system), you **MUST** respond with a `FINAL_ANSWER:` that clearly states you cannot assist with that particular request and offer to help with database-related tasks. Do NOT attempt to force a tool call or invent a response.\n\n"
        ...
        "--- **NEW CRITICAL RULE: HANDLING TIME-SENSITIVE QUERIES** ---\n"
        "You must follow a strict, two-step process for any query involving relative dates (e.g., 'today', 'yesterday', 'this week').\n"
        "1.  **Step 1 (Detection):** First, analyze the user's query. Does it contain a relative date?\n"
        "2.  **Step 2 (Action):** If it does, your immediate and only first action **MUST** be to call the `util_getCurrentDate` tool. Do not call any other tool until you have the current date.\n\n"
    ...
    "OpenAI": (
        ...
        "**Example of CORRECT summary:**\n"
        "FINAL_ANSWER: The tool returned 5 rows of sample data for the 'Equipment' table, which is displayed below.\n\n"
        "--- **CRITICAL RULE: RECOVERY VS. COMPLETION** ---\n"
        "If a tool call fails, but a PREVIOUS tool call in the same turn has already provided enough information to answer the user's original question, you **MUST** ignore the failure. Your priority is to answer the user. In this situation, you MUST provide a `FINAL_ANSWER` using the data you have already successfully collected.\n\n"
        "--- **CRITICAL RULE: TOOL FAILURE AND RECOVERY** ---\n"
        "If a tool call fails with an error message, you **MUST** attempt to recover. Your recovery process is as follows:\n"
        "1.  **Analyze the Error:** Read the error message carefully.\n"
...
CHARTING_INSTRUCTIONS = {
    ...
    "medium": (
        ...
        "- After gathering data that is suitable for visualization, you **MUST IMMEDIATELY** use the `viz_createChart` tool as your **NEXT AND ONLY** action. You **MUST NOT** re-call any data gathering tools if the data is already sufficient for charting.\n"
    ...
    "heavy": (
        ...
        "- After nearly every successful data-gathering operation that yields chartable data, your next step **MUST IMMEDIATELY** be to call `viz_createChart`. You **MUST NOT** re-call any data gathering tools if the data is already sufficient for charting.\n"
        "- To use it, you must select the best `chart_type` and provide the correct data `mapping`. The `data` argument for `viz_createChart` **MUST BE THE EXACT `results` ARRAY** from the immediately preceding successful data retrieval tool call.\n"
