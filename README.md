...
* [Overview: A Superior Approach](https://www.google.com/search?q=%23overview-a-superior-approach)
* [How It Works: Architecture](https://www.google.com/search?q=%23how-it-works-architecture)
* [Key Features](https://www.google.com/search?q=%23key-features)
* [Agent Workflows and Prompt Contracts](https://www.google.com/search?q=%23agent-workflows-and-prompt-contracts)
  * [How Workflows are Executed](https://www.google.com/search?q=%23how-workflows-are-executed)
  * [The Contract for Prompt Workflows](https://www.google.com/search?q=%23the-contract-for-prompt-workflows)
* [Installation and Setup Guide](https://www.google.com/search?q=%23installation-and-setup-guide)
  * [Prerequisites](https://www.google.com/search?q=%23prerequisites)
  * [Step 1: Clone the Repository](https://www.google.com/search?q=%23step-1-clone-the-repository)
  * [Step 2: Set Up the Python Environment](https://www.google.com/search?q=%23step-2-set-up-the-python-environment)
  * [Step 3: Create the Project Configuration File](https://www.google.com/search?q=%23step-3-create-the-project-configuration-file)
  * [Step 4: Install the Application in Editable Mode](https://www.google.com/search?q=%23step-4-install-the-application-in-editable-mode)
  * [Step 5: Configure API Keys (Optional)](https://www.google.com/search?q=%23step-5-configure-api-keys-optional)
* [Running the Application](https://www.google.com/search?q=%23running-the-application)
  * [Standard Mode](https://www.google.com/search?q=%23standard-mode)
  * [Developer Mode: Unlocking Models](https://www.google.com/search?q=%23developer-mode-unlocking-models)
* [User Guide](https://www.google.com/search?q=%23user-guide)
* [Troubleshooting](https://www.google.com/search?q=%23troubleshooting)
* [License](https://www.google.com/search?q=%23license)
* [Author & Contributions](https://www.google.com/search?q=%23author--contributions)
...
The Trusted Data Agent features a sophisticated workflow engine that enables it to execute multi-step tasks deterministically. This is triggered when the LLM selects a **Prompt** from the available capabilities, rather than a single-action **Tool**.

Here’s how it works inside the `PlanExecutor`:

1. **Workflow Identification**: When the LLM's response contains a `prompt_name`, the executor recognizes this as a request to initiate a multi-step workflow.
2. **Plan Parsing**: The `WorkflowManager` class is instantiated. It parses the entire text content of the selected MCP Prompt, looking for Markdown headers (`## Phase X...`) to identify the distinct stages of the plan.
3. **Deterministic Execution**: The `PlanExecutor` then iterates through the parsed steps sequentially. For each step, it calls the specified tool with the correct arguments. This process is "headless"—it does **not** need to call the LLM again to decide on the next action.
4. **Efficiency and Reliability**: This deterministic, parse-then-execute model is significantly faster, cheaper, and more reliable than a purely conversational agent that must stop and ask the LLM for guidance after every single tool call. It ensures that well-defined workflows are executed exactly as designed.

### The Contract for Prompt Workflows

To ensure the agent can reliably parse and execute these multi-step workflows, all MCP Prompts intended for workflow execution **must** adhere to the following structural and semantic contract.
...
* **Overall Structure**: The prompt must be divided into logical **Phases** using Markdown H2 headers (`##`). Each phase represents a major stage of the workflow.
  ` ` `
  ...
  ...
  ` ` `
* **Steps**: Within each phase, individual actions **must** be specified as bullet points (`-`). Each bullet point represents a single step in the sequence.
  ` ` `
  ...
  ` ` `
* **Loops**: Loops **must** be explicitly declared using one of the following keyword phrases:
  * `Cycle through the list of [items]...`
  * `For each [item] in the list...`

  The placeholder `[items]` (e.g., "tables", "columns") is crucial. The steps to be executed inside the loop should be defined in an indented list immediately following the loop declaration.
  ` ` `
  ## Phase 2 - Collect Table Information
...
* **Explicit Tool Naming**: Every action step **must** explicitly name the tool to be used with the phrase `...using the [tool_name] tool`. The `[tool_name]` must match the exact name of a tool available on the MCP server.
* **Parameter Placeholders**: When a tool's argument should be filled in dynamically (e.g., from user input or the context of a loop), use curly braces `{}`. The executor's context management will automatically substitute these at runtime.
  * If a loop is `For each table...`, the steps inside can refer to `{table_name}`.
  * If a loop is `For each column...`, the steps inside can refer to `{column_name}`.

#### 4. Example: Complete Workflow Prompt

This is an example of a well-formed workflow prompt that adheres to all guidelines.
